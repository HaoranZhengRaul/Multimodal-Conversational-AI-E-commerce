...
def download_file(url, save_path):
    ...
    if response.status_code == 200:
        ...
        st.error(f"Failed to download {url}: {response.status_code}")
        return None
...
def load_faiss_indices():
    ...
    for name, url in indices.items():
        ...
        downloaded_path = download_file(url, save_path)
        if downloaded_path:
            loaded_indices[name] = faiss.read_index(downloaded_path)
        else:
            st.error(f"Could not load the FAISS index for {name}.")
