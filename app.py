...
def load_faiss_indices():
    text_index = faiss.read_index("path/to/text_only.index")
    image_index = faiss.read_index("path/to/image_only.index")
    full_index = faiss.read_index("path/to/multimodal_embeddings.index")
    return text_index, image_index, full_index
...
def generate_text_embeddings(texts):
    ...
    with torch.no_grad():
        return clip_model.get_text_features(**inputs).numpy()
...
def generate_image_embeddings(images):
    ...
    for image in images:
        ...
        with torch.no_grad():
            embeddings.append(clip_model.get_image_features(**inputs).numpy())
    return np.vstack(embeddings)

def retrieve_items(index, embedding, top_k=5):
    """Retrieve top-k items from a FAISS index."""
